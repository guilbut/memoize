...
max_bytes = 2**31 - 1
# disk_cache_dir = os.path.join(tempfile.gettempdir(), "memoize_to_disk_cache")
disk_cache_dir = os.path.join(
    os.path.abspath(os.path.dirname(sys.argv[0])), "__memoize__"
)
user_ignore_files: Set[str] = set()  # should contain / not \
ignore_folders = [os.path.dirname(os.__file__), get_python_lib()]
strings_filter_out = set(
    [
        os.path.normpath("python\\debugpy\\_vendored\\pydevd"),
        "__pycache__",
    ]
)
skip_check_global_changed = set(["<class 'numba.core.registry.CPUDispatcher'>"])
...
hashing_func_map: Dict[Any, Callable] = {
    str: str,
    int: str,
...
def get_globals_from_code(code: types.CodeType) -> List[str]:
    ...
    extanded_args = opmap["EXTENDED_ARG"]
    ...
    for c in op:
        ...
        elif c == extanded_args:
            # raise BaseException("not sure what to do")
            # extarg = (next(op) + next(op) * 256) * 65536
            continue
        elif c == opmap["CALL_FUNCTION"]:
            # raise BaseException("not sure how to get the function name")
            # globs.remove(func)
            pass
...
def get_function_from_frame(frame: types.FrameType) -> Optional[Callable]:
    ...
    if "self" in frame.f_locals:  # rely on convention it ia called self
...
def get_global_hash(
    ...
    if str(variable_type) in hashing_func_map:
        ...
        try:
            ...
            if frame is None:
                raise BaseException(f"Could not pickle global variable {name}")
            else:
...
class Tracer:
    ...
    def add_data_dependency(self, file_path: str) -> None:
        ...
        if any(
            [file_path.lower().startswith(folder.lower()) for folder in ignore_folders]
        ):
            return
        ...
        last_modified_date_str = str(
            datetime.datetime.fromtimestamp(last_modified_date)
        )
        dep = DataDependency(
            file_path=file_path, last_modified_date_str=last_modified_date_str
        )
    ...
    def tracefunc(self, frame: types.FrameType, event: str, arg: str) -> None:
        ...
        if event == "call":
            ...
            if (
                not any(
                    [
                        filename.lower().startswith(folder.lower())
                ...
                if func_key not in self.function_qualified_name:
                    self.function_qualified_name[
                        func_key
                    ] = get_function_qualified_name_from_frame(frame)
                ...
                if func_key not in self.function_alias:
                    ...
                    if func_key not in self.function_modules:
                        ...
                        assert (
                            module is not None
                        ), f"Module not found for function {function_name} in {filename}"
...
def dependency_changed(func: Callable, all_dependencies: dict) -> bool:
    ...
    if all_dependencies["random_states"] is not None:
        random_state_before = {
            "numpy": base64.b64encode(pickle.dumps(np.random.get_state())).decode(
                "utf-8"
            )
        }
        for name in ["numpy"]:
            if (
                not random_state_before[name]
                == all_dependencies["random_states"]["before"][name]
            ):
    ...
    for entry_dict in all_dependencies["code"]:
            ...
            for name in function_qualified_name.split("."):
                ...
                if new_node is None:
                    ...
                    if getattr(node, "__qualname__", None):
                        print_change(f"Could not find {name} in {node.__qualname__}.")
                    else:
                        print_change(f"Could not find {name} in {node}")
        ...
        for name, value in global_variables.items():
            if dep_global_vars is None or name not in dep_global_vars:
                print_change(
                    f"Global variable {name} used in {function_qualified_name} not found"
                )
                return True
            ...
            if hash_str != value:
                print_change(
                    f"Global variable {name} used in {function_qualified_name} has been modified"
                )
                return True
        ...
        for name, value in closure_variables.items():
            ...
            if hash_str != value:
                print_change(
                    f"Closure variable {name} used in {function_qualified_name} has been modified"
                )
                return True
        try:
...
def get_dependencies_runtime(
...
) -> Tuple[Any, FunctionDependencies]:
    ...
    inherited_dependencies_counters_copy = copy.copy(
        tracer.inherited_dependencies_counters
    )
    ...
    for func_key, val in tracer.code_dependencies_counters.items():
        if (
            func_key in code_dependencies_counters_copy
            and code_dependencies_counters_copy[func_key] == val
        ):
            continue
    ...
    for key, val in tracer.data_dependencies_counters.items():
        if (
            key in data_dependencies_counters_copy
            and data_dependencies_counters_copy[key] == val
        ):
            continue
    ...
    for key, val in tracer.inherited_dependencies_counters.items():
        if (
            key in inherited_dependencies_counters_copy
            and inherited_dependencies_counters_copy[key] == val
        ):
            continue
...
def memoize(func: Callable[P, R]) -> Callable[P, R]:
    ...
    def _memoize_wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        result_file = os.path.join(
            disk_cache_dir, f"{full_qualified_name_file}_{hash_str_short}_result.pkl"
        )
        ...
        if exists(dependencies_file):
            print(
                f"Found potential memoized results in {dependencies_file}.\nchecking dependencies..."
            )
            use_cached = True
            ...
            if use_cached:
                print(
                    f"Loading memoized results for function {full_qualified_name_file} from {result_file}"
                )
                try:
                    ...
                    use_cached = False
                    print(
                        f"WARNING: Could not unpickle the data from {result_file}, probably due to code change."
                    )
            ...
            if use_cached:
                ...
                if all_dependencies["random_states"] is not None:
                    np.random.set_state(
                        pickle.loads(
                            binascii.a2b_base64(
                                all_dependencies["random_states"]["after"][
                                    "numpy"
                                ].encode("utf-8")
                            )
                        )
        ...
        print(f"Result for {full_qualified_name_file} saved in\n{result_file}")
...
def memoized_open_wrapper(
    filename: str, mode: str, encoding: Optional[str] = None
) -> IO[Any]:
    if mode in ["r", "rb"]:
        if not os.path.exists(filename):
